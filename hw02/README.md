...
Так как время отправления и прибытия по UTC, то можно также добавить ограничение CHECK (tdown > atd).
...
|------|-----------------------------------------------|--------|-------------|
| id   | номер записи                                  | 1      | PK          |
| pos  | позиция                                       | LEFT   | NOT NULL    |
| loc  | расположение на крыле (ONE/INTERNAL/EXTERNAL) | ONE    | NOT NULL    |
